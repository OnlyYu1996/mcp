    ...
    try {
        ...
        for (const selector of resultSelectors) {
            ...
            $(selector).each((index, element) => {
                ...
                if ((!title && !snippet) || $(element).hasClass('b_ad'))
                    return;
                // 创建唯一ID
                ...
                // 保存到结果映射中
                const result = { id, title, link, snippet };
        ...
        if (results.length === 0) {
            ...
            $('a').each((index, element) => {
                ...
                if (isLikelyResult) {
                    ...
                    const result = { id, title, link: fullLink, snippet };
                    searchResults.set(id, result);
                    results.push(result);
                }
            });
 ...
 */
async function fetchWebpageContent(resultId) {
    try {
        // 从搜索结果映射中获取URL
